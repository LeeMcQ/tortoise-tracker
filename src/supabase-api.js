...
export class SupabaseAPI {
  constructor(){ const c=window.NAUTILUS_CONFIG; this.url=(c.supabaseUrl||'').replace(/\/$/,''); this.key=c.supabasePublishableKey||''; this.session=JSON.parse(localStorage.getItem('nautilus-supabase-session')||'null'); }
  ...
  async request(path,{method='GET',body,headers={}}={}){ const r=await fetch(`${this.url}${path}`,{method,headers:this.headers(body instanceof FormData?headers:{'Content-Type':'application/json',...headers}),body:body instanceof FormData?body:body==null?undefined:JSON.stringify(body)}); if(!r.ok) throw new Error(await r.text()); const ct=r.headers.get('content-type')||''; return ct.includes('json')?r.json():r.text(); }
  async rpc(name,params={}){ return this.request(`/rest/v1/rpc/${name}`,{method:'POST',body:params}); }
  async publicAnimals(){ return this.rpc('list_public_animals',{}); }
  async publicAnimal(publicId){ const rows=await this.rpc('get_public_animal',{p_public_id:publicId}); return Array.isArray(rows)?rows[0]||null:rows; }
  ...
  async login(email,password){ const data=await this.request('/auth/v1/token?grant_type=password',{method:'POST',body:{email,password}}); this.session=data; localStorage.setItem('nautilus-supabase-session',JSON.stringify(data)); return data; }
  async refresh(){ if(!this.session?.refresh_token) throw new Error('No refresh token'); const data=await this.request('/auth/v1/token?grant_type=refresh_token',{method:'POST',body:{refresh_token:this.session.refresh_token}}); this.session=data; localStorage.setItem('nautilus-supabase-session',JSON.stringify(data)); return data; }
  logout(){ this.session=null; localStorage.removeItem('nautilus-supabase-session'); }
  ...
  async verifyFactor(factorId,challengeId,code){ const data=await this.request(`/auth/v1/factors/${factorId}/verify`,{method:'POST',body:{challenge_id:challengeId,code}}); if(data?.access_token){ this.session={...this.session,...data}; localStorage.setItem('nautilus-supabase-session',JSON.stringify(this.session)); } return data; }
