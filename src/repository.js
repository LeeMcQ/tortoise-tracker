...
export const Repo={
  ...
  async publicHistory(publicId){
    ...
    const a=DemoDB.animalByPublicId(publicId); if(!a)return[]; const rows=DemoDB.observationsForAnimal(a.id).filter(isPubliclyReleased); return rows.map((o,i)=>publicObservation(o,{latest:i===rows.length-1}));
  },
  async publicMap(){
    if(!isDemo()) return api.publicMap();
    const result=[]; for(const a of DemoDB.activeAnimals()){ const rows=DemoDB.observationsForAnimal(a.id).filter(isPubliclyReleased); rows.forEach((o,i)=>result.push({...publicObservation(o,{latest:i===rows.length-1}),name:a.name,public_id:a.public_id})); } return result;
  },
